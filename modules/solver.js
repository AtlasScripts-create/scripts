(function() {
                ...
                res.clone().json().then(json => {
                    ...
                    if (itemData) {
                        ...
                        if (respostas.length > 0) {
                            window.va_updateStatus("PRONTO", "#0f0");
                            if (window.va_ativo) setTimeout(resolver, 600);
                        }
                    }
                }).catch(()=>{});
            }
    ...
    window.addEventListener('keydown', (e) => {
        if (e.key === "F4") {
            ...
            window.va_updateStatus(window.va_ativo ? "AUTO-ON" : "AUTO-OFF", window.va_ativo ? "#0f0" : "#f00");
            if (window.va_ativo) resolver();
        }
    });
})();
